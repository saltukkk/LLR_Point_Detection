...
from helper_functions.least_squares import lsf as lsf1
from helper_functions.mask_utils import get_contpt
from helper_functions.utils import get_bottom_width, determine_mid_of_joint_x
...
def calc_ankle_level(mask, vert_tol=0.025, horiz_tol=0.25):
    ...
    _, indices_level = get_contpt(mask[int(np.mean(contour[0])) :])
...
def determine_mid_of_ankle_y(mask, leave_out_percentage=0.15):
    lsf = lsf1(mask, (leave_out_percentage,leave_out_percentage))
    contours = get_contpt(mask)
